...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch) -> None:
        ...
        if self.rank == 0:
            ...
            pbar = PBar(
                self.steps_per_epoch, 10, stateful_metrics=self.stateful_metrics
            )
        ...
        for dl_step, batch in enumerate(self.train_dl):
            ...
            wer = torch.tensor(
                self.compute_metric(outputs.logits.detach(), batch["labels"])
            )

            # Optimize step
            if (dl_step + 1) % self.gradient_accumulation_steps == 0 or dl_step == len(
                self.train_dl
            ) - 1:
                # compute grad norm for monitoring
                grad_norm = self.get_grad_norm(
                    self.model.parameters(), scale=self.scaler.get_scale()
                )

                # gradient clipping
                self.scaler.unscale_(self.optimizer)
                torch.nn.utils.clip_grad_norm_(
                    self.model.parameters(), self.max_clip_grad_norm
                )
    ...
    def _valid_epoch(self, step) -> Dict[str, Union[Any, float]]:
        ...
        for batch in tqdm(
            self.val_dl, total=len(self.val_dl), disable=not self.rank == 0
        ):
            with torch.no_grad():
                with autocast(enabled=self.use_amp):
                    outputs = self.model(**batch)
            ...
            val_logs["wer"] += torch.tensor(
                self.compute_metric(outputs.logits, batch["labels"])
            ) / len(self.val_dl)
        ...
        val_logs = {
            k: v.item() if hasattr(v, "item") else v for k, v in val_logs.items()
        }
        return val_logs
